...
function App() {
  ...
  const loadWeekPicks = (weekKey) => {
    ...
    if (existingPick && existingPick.predictions) {
      ...
      const picksCopy = JSON.parse(JSON.stringify(existingPick.predictions));
      setPredictions(picksCopy);
      setOriginalPicks(JSON.parse(JSON.stringify(existingPick.predictions)));
      setHasUnsavedChanges(false);
      console.log('📂 Loaded picks for', weekKey, '- Setting hasUnsavedChanges to FALSE');
    } else {
      setPredictions({});
      setOriginalPicks({});
      setHasUnsavedChanges(false);
      console.log('📂 No existing picks for', weekKey, '- Setting hasUnsavedChanges to FALSE');
    }
  };
  ...
  useEffect(() => {
    ...
    console.log('🔍 useEffect: Checking for changes...');
    console.log('🔍 predictions:', JSON.stringify(predictions));
    console.log('🔍 originalPicks:', JSON.stringify(originalPicks));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('✅ SUBMIT: Setting hasUnsavedChanges to FALSE after successful submit');
      console.log('✅ SUBMIT: originalPicks updated to match predictions');
